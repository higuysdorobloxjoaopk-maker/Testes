-- STREAMING_CHUNK:Inicializando serviços e variáveis locais do sistema...
...
-- STREAMING_CHUNK:Definindo a função principal de criação da janela (Window)...
function lutyI:Window(winconfig)
local function initui()
local G2L = {};
        -- StarterGui.Panel
        G2L["1"] = Instance.new("ScreenGui", game:GetService("CoreGui"));
        G2L["1"]["ResetOnSpawn"] = false;
        G2L["1"]["Name"] = winconfig.Name or "lutyI_Interface";
        ...
        -- STREAMING_CHUNK:Adicionando animação de entrada e cantos arredondados...
        local uiScale = Instance.new("UIScale", G2L["2"])
        uiScale.Scale = 0
        ts:Create(uiScale, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Scale = 1}):Play()
        ...
        -- STREAMING_CHUNK:Configurando os detalhes visuais da aba lateral...
        ...
        G2L["7"]["Text"] = winconfig.Title or "lutyI"
        G2L["7"]["Name"] = [[Header]];
        ...
        -- STREAMING_CHUNK:Construindo a lista de guias (Tabs)...
   ...
   G2L["ut"]["Padding"] = UDim.new(0,2);
        ...
        -- STREAMING_CHUNK:Criando o campo de pesquisa (Search Box)...
        ...
        -- STREAMING_CHUNK:Finalizando a estrutura base e aplicando bordas (UIStroke)...
...
-- STREAMING_CHUNK:Configurando os atalhos de teclado para visibilidade...
game:GetService("UserInputService").InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == winconfig.Key then
                if panel.Visible then
                        ts:Create(panel.UIScale, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.In), {Scale = 0}):Play()
                        task.wait(0.3)
                        panel.Visible = false
                else
                        panel.Visible = true
                        ts:Create(panel.UIScale, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Scale = 1}):Play()
                end
        end
end)

-- STREAMING_CHUNK:Implementando o botão de Mobile-Friendly atualizado...
if IsOnMobile then
        ...
        MobileToggle.Name = "MobileToggle_lutyI"
        ...
        Trigger.Size = UDim2.new(0, 45, 0, 45)
        -- Nova imagem para abrir (Menu Hambúrguer)
        Trigger.Image = "rbxassetid://10888331510" 
        
        -- Animação ao carregar o botão mobile
        local mScale = Instance.new("UIScale", Trigger)
        mScale.Scale = 0
        ts:Create(mScale, TweenInfo.new(0.6, Enum.EasingStyle.Bounce), {Scale = 1}):Play()
        ...
        Trigger.MouseButton1Click:Connect(function()
                ...
                ts:Create(mScale, TweenInfo.new(0.1, Enum.EasingStyle.Quad), {Scale = 0.8}):Play()
                task.wait(0.1)
                ts:Create(mScale, TweenInfo.new(0.1, Enum.EasingStyle.Quad), {Scale = 1}):Play()
        end)
        ...
        -- STREAMING_CHUNK:Lógica para permitir o arrasto do botão mobile...
...
-- STREAMING_CHUNK:Conectando a lógica da barra de pesquisa (Search Box)...
...
-- STREAMING_CHUNK:Inicializando as utilidades de Redimensionar (Resizable) e Arrastar (Draggable)...
pcall(function() -- ui stuff
        local function handler()
                ...
                function module.makeResizable(obj:GuiObject, minSize)
                        ...
                        -- STREAMING_CHUNK:Eventos de redimensionamento do painel...
                        ...
                        dragger.MouseEnter:Connect(function() finishResize(DRAGGER_TRANSPARENCY) end)
                        dragger.MouseLeave:Connect(function() finishResize(1) end)                
                        dragger.MouseButton1Up:Connect(function() finishResize(DRAGGER_TRANSPARENCY) end)        
                end
...
-- STREAMING_CHUNK:Declarando o módulo de criação das Guias (Tabs)...
local window = {}
...
function window:AddTab(tabconfig)
   ...
   G2L["dsfd"]["SortOrder"] = Enum.SortOrder.LayoutOrder
        ...
        -- STREAMING_CHUNK:Personalizando botões das Guias (Tabs)...
        ...
        -- STREAMING_CHUNK:Logica para mudar entre as Guias (Tabs)...
        ...
        -- STREAMING_CHUNK:Declarando funções de Seção (Section)...
        function tab:Section(name)
                ...
                -- STREAMING_CHUNK:Configurando o botão de esconder (Hide) seção...
                ...
                -- STREAMING_CHUNK:Adicionando o Componente: Botão (Button)...
                function section:Button(btnconfig)
                        ...
                        G2L["e"]["Size"] = UDim2.new(1, -10, 0, 30); -- Size increased slightly for mobile
                ...
                -- STREAMING_CHUNK:Adicionando o Componente: Alternador (Toggle)...
                function section:Toggle(togconfig)
                        ...
                        G2L["14"]["Size"] = UDim2.new(1, -10, 0, 30);
                ...
                -- STREAMING_CHUNK:Adicionando o Componente de Entrada de Texto (Input)...
                function section:Input(inconfig)
                        ...
                        G2L["1c"]["Size"] = UDim2.new(1, -10, 0, 30);
                ...
                -- STREAMING_CHUNK:Adicionando o Componente de Texto Simples (Label)...
                function section:Label(text)
                        -- StarterGui.Panel.UI.side.Header
                        local G2L_Label = {}
                        G2L_Label["7"] = Instance.new("TextLabel", items);
                        G2L_Label["7"]["BorderSizePixel"] = 0;
                        G2L_Label["7"]["RichText"] = true;
                        G2L_Label["7"]["BackgroundColor3"] = Color3.fromRGB(255, 255, 255);
                        G2L_Label["7"]["FontFace"] = Font.new([[rbxasset://fonts/families/GothamSSm.json]], Enum.FontWeight.SemiBold, Enum.FontStyle.Normal);
                        G2L_Label["7"]["TextSize"] = 14;
                        G2L_Label["7"]["TextColor3"] = Color3.fromRGB(228, 228, 228);
                        G2L_Label["7"]["Size"] = UDim2.new(1, 0, 0, 23);
                        G2L_Label["7"]["BorderColor3"] = Color3.fromRGB(0, 0, 0);
                        G2L_Label["7"]["Text"] = text
                        G2L_Label["7"]["Name"] = [[Header]];
                        G2L_Label["7"]["BackgroundTransparency"] = 1;

                        -- StarterGui.Panel.UI.side.Header.UIPadding
                        G2L_Label["8"] = Instance.new("UIPadding", G2L_Label["7"]);
                        G2L_Label["8"]["PaddingLeft"] = UDim.new(0, 10);
                end

                -- STREAMING_CHUNK:Adicionando o NOVO Componente (Slider)...
                function section:Slider(slidconfig)
                        local G2L_S = {}
                        -- Container principal do Slider
                        G2L_S["Container"] = Instance.new("Frame", items)
                        G2L_S["Container"].BackgroundColor3 = Color3.fromRGB(17, 17, 17)
                        G2L_S["Container"].Size = UDim2.new(1, -10, 0, 45)
                        G2L_S["Container"].Name = slidconfig.Name
                        
                        Instance.new("UICorner", G2L_S["Container"]).CornerRadius = UDim.new(0, 5)
                        
                        local stroke = Instance.new("UIStroke", G2L_S["Container"])
                        stroke.Color = Color3.fromRGB(37, 37, 37)
                        stroke.Thickness = 0.6
                        stroke.Transparency = 0.44

                        -- Header (Nome do Slider)
                        G2L_S["Header"] = Instance.new("TextLabel", G2L_S["Container"])
                        G2L_S["Header"].BackgroundTransparency = 1
                        G2L_S["Header"].Size = UDim2.new(1, -50, 0, 20)
                        G2L_S["Header"].Position = UDim2.new(0, 10, 0, 5)
                        G2L_S["Header"].Font = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold)
                        G2L_S["Header"].TextSize = 13
                        G2L_S["Header"].TextColor3 = Color3.fromRGB(228, 228, 228)
                        G2L_S["Header"].TextXAlignment = Enum.TextXAlignment.Left
                        G2L_S["Header"].Text = slidconfig.Name

                        -- Texto de Valor (Value)
                        G2L_S["Value"] = Instance.new("TextLabel", G2L_S["Container"])
                        G2L_S["Value"].BackgroundTransparency = 1
                        G2L_S["Value"].Size = UDim2.new(0, 40, 0, 20)
                        G2L_S["Value"].Position = UDim2.new(1, -50, 0, 5)
                        G2L_S["Value"].Font = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold)
                        G2L_S["Value"].TextSize = 13
                        G2L_S["Value"].TextColor3 = Color3.fromRGB(255, 179, 26) -- Accent Color
                        G2L_S["Value"].TextXAlignment = Enum.TextXAlignment.Right
                        G2L_S["Value"].Text = tostring(slidconfig.Default or slidconfig.Min)

                        -- Fundo da Barra (Bar Background)
                        G2L_S["BarBg"] = Instance.new("Frame", G2L_S["Container"])
                        G2L_S["BarBg"].BackgroundColor3 = Color3.fromRGB(14, 14, 14)
                        G2L_S["BarBg"].Size = UDim2.new(1, -20, 0, 6)
                        G2L_S["BarBg"].Position = UDim2.new(0, 10, 0, 30)
                        Instance.new("UICorner", G2L_S["BarBg"]).CornerRadius = UDim.new(1, 0)

                        -- Preenchimento da Barra (Bar Fill)
                        G2L_S["BarFill"] = Instance.new("Frame", G2L_S["BarBg"])
                        G2L_S["BarFill"].BackgroundColor3 = Color3.fromRGB(255, 179, 26)
                        
                        local minVal = slidconfig.Min or 0
                        local maxVal = slidconfig.Max or 100
                        local defVal = slidconfig.Default or minVal
                        local defaultPercent = math.clamp((defVal - minVal) / (maxVal - minVal), 0, 1)
                        
                        G2L_S["BarFill"].Size = UDim2.new(defaultPercent, 0, 1, 0)
                        Instance.new("UICorner", G2L_S["BarFill"]).CornerRadius = UDim.new(1, 0)

                        -- Trigger de Arrastar/Toque (Trigger)
                        G2L_S["Trigger"] = Instance.new("TextButton", G2L_S["Container"])
                        G2L_S["Trigger"].BackgroundTransparency = 1
                        G2L_S["Trigger"].Text = ""
                        G2L_S["Trigger"].Size = UDim2.new(1, 0, 1, 0)

                        -- Lógica do Arraste Mobile-Friendly
                        local dragging = false
                        local uis = game:GetService("UserInputService")

                        local function updateSlider(input)
                                local pos = math.clamp(input.Position.X - G2L_S["BarBg"].AbsolutePosition.X, 0, G2L_S["BarBg"].AbsoluteSize.X)
                                local percent = pos / G2L_S["BarBg"].AbsoluteSize.X
                                local val = math.floor(minVal + ((maxVal - minVal) * percent))
                                
                                ts:Create(G2L_S["BarFill"], TweenInfo.new(0.05), {Size = UDim2.new(percent, 0, 1, 0)}):Play()
                                G2L_S["Value"].Text = tostring(val)
                                pcall(function() slidconfig.Callback(val) end)
                        end

                        G2L_S["Trigger"].InputBegan:Connect(function(input)
                                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                                        dragging = true
                                        updateSlider(input)
                                end
                        ...
                        G2L_S["Trigger"].InputEnded:Connect(function(input)
                                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                                        dragging = false
                                end
                        end)
